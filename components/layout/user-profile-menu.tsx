...
import { clearStoredSession, getStoredAppRole, getStoredUser } from "@/lib/services/session";
...
export function UserProfileMenu() {
  ...
  useEffect(() => {
    ...
    setStoredUserName(getStoredUser()?.name ?? null);
  }, [pathname]);

  const user = usersByRole[storedRole ?? resolveRoleFromPathname(pathname)];
  const displayName = storedUserName ?? user.name;

  return (
    <button
      className="flex min-h-14 items-center gap-3 rounded-2xl border border-border/80 bg-white px-4 py-3 text-left shadow-soft transition hover:-translate-y-0.5"
      onClick={() => {
        clearStoredSession();
